...
init({Ref, Transport, _Opts}) ->
    error_logger:info_msg("socket protocol init"),
    {ok, Socket} = ranch:handshake(Ref),
	ok = Transport:setopts(Socket, [{active, once}, {packet, 2}]),
	gen_server:enter_loop(?MODULE, [], #{socket=>Socket, transport=>Transport, account=>""}).
...
handle_info({tcp, Socket, Bin}, #{transport:=Transport, socket:=Socket}=State) ->
        ...
        [AccInfo] -> 
            if 
                ...
                true ->
                    ...
                    db:update("account", #{account=>list_to_binary(Acc)}, #{login_time=>common:timestamp()}),
...
do_unregister(#account_info{account=Acc}=AccInfo, _Msg) ->
    ets:insert(account, AccInfo#account_info{delete_time=common:timestamp()}),
    db:update("account", #{account=>list_to_binary(Acc)}, #{delete_time=>common:timestamp()}),
...
do_register(Msg) ->
    ...
    case db:find_account_info(Acc) of
        [] -> 
            db:insert_one("account", #{account=>list_to_binary(Acc), passwd=>list_to_binary(Pass), register_time=>common:timestamp(), delete_time=>0, 
                    user_info=>#{account=>list_to_binary(Acc)}}),
...
disconnect([]) -> 
    void;
disconnect(#account_info{account=Acc}=AccInfo) -> 
    on_user_offline(AccInfo),
    ets:delete(account, Acc),
    db:update("account", #{account=>list_to_binary(Acc)}, #{logout_time=>common:timestamp()}).
